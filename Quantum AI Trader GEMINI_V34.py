...
from sklearn.ensemble import GradientBoostin# -*- coding: utf-8 -*-
# AI-Powered QuantumTrade Bot (TEMA+SuperTrend, ML Destekli, Full Çalışır)
...
def get_ml_market_direction(df, model=None, scaler=None, features=None, retrain=False, lookforward=4, threshold=0.0075, prob_long=0.52, prob_short=0.48):
    ...
    if pred_prob > prob_long:
        return 'LONG', pred_prob, model, scaler
    elif pred_prob < prob_short:
        return 'SHORT', pred_prob, model, scaler
    else:
        return 'NEUTRAL', pred_prob, model, scaler

# ========== Klasik Skor Bazlı Market Direction Predictor ==========
def get_classic_market_direction(df, ema_fast=5, ema_slow=20, rsi_period=14, rsi_ob=70, rsi_os=30, macd_fast=12, macd_slow=26, macd_signal=9, vwap=True, score_threshold=3):
    # EMA, RSI, MACD, VWAP skorlamasıyla yön tahmini
    close = df['close']
    ...
    for combo in combinations:
        ...
        for method in method_list:
            ...
            for dt in dt_range:
                ...
                # Gelecek pencere
                future_start = dt
                future_end = dt + timedelta(hours=future_window_hours)
                future_df = df[(df.index >= future_start) & (df.index < future_end)]
                if len(future_df) < 2:
                    continue
                price_start = future_df["open"].iloc[0]
                price_end = future_df["close"].iloc[-1]
...
if __name__ == "__main__":
    ...
    # Demo veri için (canlıda kendi fetch_data_multi ile değiştir)
    df = yf.download("ETH-USD", interval="15m", period="30d")
    df = df.rename_axis('timestamp').reset_index().set_index('timestamp')
    df = df[['Open', 'High', 'Low', 'Close', 'Volume']]
    df.columns = ['open', 'high', 'low', 'close', 'volume']
    # Grid search başlat
    asyncio.run(gridsearch_market_direction(df))gClassifier
from sklearn.preprocessing import StandardScaler
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense, Input
from tensorflow.keras.optimizers import Adam
...
def get_ml_market_direction(df, model=None, scaler=None, features=None, retrain=False):
    """
    df: Son N barı içeren DataFrame (örn: 50-200 bar)
    model: Daha önce eğitilmiş bir XGBoost modeli (veya None)
    scaler: StandardScaler objesi (veya None)
    features: Feature isim listesi
    retrain: True ise, son 80% ile yeniden eğitir
    Dönüş: 'LONG', 'SHORT', 'NEUTRAL', model, scaler
    """
    df_feat = ml_feature_engineering(df)
    features = features or [
        "ema_diff", "ema_cross_up", "ema_cross_down",
    ...
    # Etiketle: Son N barın sonundaki fiyat, N bar sonraki fiyat ile karşılaştırılır
    LOOKFORWARD = 4
    THRESHOLD = 0.0075
    df_feat['target'] = np.nan
    for idx in range(len(df_feat) - LOOKFORWARD):
    ...
    X_last = scaler.transform([df_feat[features].iloc[-1].values])
    pred_prob = model.predict_proba(X_last)[0][1]
    # Sinyal eşikleri: ister parametre olarak, ister sabit
    if pred_prob > 0.52:
        return 'LONG', model, scaler
    elif pred_prob < 0.48:
        return 'SHORT', model, scaler
    else:
        return 'NEUTRAL', model, scaler
